...
def augmentation_depth(depth, bbox, data_split, do_flip=False):
    if data_split == 'train':
        scale, rot, _ = get_aug_config()
    else:
        scale, rot = 1.0, 0.0
    depth, _, _ = generate_patch_image(depth, bbox, scale, rot, do_flip, cfg.input_img_shape)
